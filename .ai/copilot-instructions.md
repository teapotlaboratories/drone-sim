...
[`../docs/reference/`](../docs/reference/) (the design) first. Key rules:
...
Assembled from pinned upstreams (PX4, Pegasus, Isaac ROS, EGO-Planner, Cosys-AirSim,
vLLM) — the glue is the original work; don't hand-roll what a component provides. The
dominant risk is **version coupling**: two PX4 trees (v1.16.x for Lane A + real; v1.14.3
for Pegasus), Isaac Python 3.11 vs ROS 2 Jazzy 3.12, `px4_msgs` branch-matched to
firmware. Lock every version in `versions.lock` before writing code.
...
target lane (headless SITL, seeded scenarios, SR/collision/latency metrics, MCAP
evidence). If you cannot verify, say so and name the blocker.
...
unreviewed. Install into the container, never the host; keep tooling and big data out of
`~` (repo `vendor/` for tooling, the 7 TB external drive for data, `/tmp` for scratch). **On any other drive, write only under
