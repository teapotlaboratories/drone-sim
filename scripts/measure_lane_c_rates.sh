#!/usr/bin/env bash
# Measure Lane C sensor rates across image-quality configurations.        (C-11)
#
...
# ForceUpdate) was validated on ONE camera at 1080p in a standalone simulator. Lane C runs two
# cameras plus a GPU-LiDAR at 640x480, and ForceUpdate renders every ENABLED camera every frame
# rather than once per request -- so its cost scales with the graph, not with the setting. Lane
...
# `lane_c_up.sh` deletes the ROS 2 container, which is where the AirSim wrapper is built.
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SETTINGS="$REPO/sim/ue5/settings.json"
OUT="${OUT:-$REPO/out/lane-c/rate-matrix}"
...
measure() {  # measure <label> <lumen> <forceupdate>
  ...
  bash "$REPO/scripts/lane_c_up.sh" >"$OUT/$label.up.log" 2>&1 \
  ...
  docker cp "$REPO/scripts/verify_lane_c_sensors.py" lane-c-ros2:/tmp/verify.py >/dev/null
