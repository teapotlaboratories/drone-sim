#!/usr/bin/env python3
"""Verify the Lane C drone actually provides what navigation needs, over the ROS 2 link.

SITL only. Nothing real is armed or flown.

WHY THIS IS NOT `ros2 topic list`
---------------------------------
Every failure this project has hit in Lane C looked healthy from the outside:
...
class Collector(Node):
    def __init__(self):
        super().__init__("lane_c_sensor_verify")
...
def main():
    ...
    if imu:
        ...
        check("IMU duplicate rate", False, dup_pct < 20.0,
              ...
              f"-> ~{distinct/SECS:.0f} Hz of real data. KNOWN: polled snapshot, see C-04 trap 3")
    else:
    ...
    # ---- report ---------------------------------------------------------------------
    print(f"\n  Lane C sensor verification - {SECS:.0f} s sample, SITL only\n")
    req_fail = 0
