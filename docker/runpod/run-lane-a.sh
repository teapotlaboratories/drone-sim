#!/usr/bin/env bash
# Fern/Runpod Lane A batch runner.
set -Eeuo pipefail
...
if ! python3 "$RUNTIME_LIB/preflight.py" \
  ...
  echo "runner: starting pinned QGroundControl datalink"
  ...
  if [[ "$runner_status" = "0" ]]; then
    ...
    status --state failed --exit-code "$runner_status" --message "Lane A smoke failed"
  fi
fi
