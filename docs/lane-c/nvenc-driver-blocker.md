...
**Blocks:** `C-17` (1080p60 video via Pixel Streaming)

**This is the second capability lost to the same driver.** The first was Isaac Sim
(`P0-09`, [`../lane-b/isaac-driver-decision.md`](../lane-b/isaac-driver-decision.md)), which
SIGSEGVs on 610.43.03 against its validated 580.65.06. A single host driver change would
address both.
...
- **ffmpeg 6.1.1** (in `drone-sim/lane-a-video`) offers `h264_nvenc` and VAAPI encoders, but
...
| Capability | State |
|---|---|
| 1080p60 video capture (`C-17`) | **blocked** — no reachable hardware encoder |
| Isaac Sim (`P0-09`, Lane B) | already deferred on the **same driver** |
...
**Nothing that flies is blocked.** This is a presentation-quality video ceiling, plus Lane B.

## 5. Options

1. **Rebase the host to an R580-series driver.** *Owner-only* — the host is ostree-immutable and
   host `sudo` is unavailable here. Would very likely restore NVENC **and** reopen Lane B, which
   is the strongest argument for it: one change, two capabilities.
...
2. Isaac Sim launches (the `P0-09` check) — confirms the other half of the trade.
3. Only then attempt `C-17`, whose remaining risks are unrelated to this blocker: headless
