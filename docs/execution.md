...
Every push to `main` runs `.github/workflows/lane-a-image.yml`. It publishes:

```text
ghcr.io/teapotlaboratories/drone-sim:lane-a-base-<main-sha>
ghcr.io/teapotlaboratories/drone-sim:lane-a-<main-sha>
ghcr.io/teapotlaboratories/drone-sim:lane-a-v1.16.0
```
...
Use the digest emitted by the successful `Lane A image` workflow:
