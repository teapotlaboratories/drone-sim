...
>
> **This is the document that chose the architecture the repo still runs.** Its central
> argument — Cosys-AirSim on Unreal, driving PX4 SITL over MAVLink while PX4's own
> `uxrce_dds_client` exposes the same `/fmu/*` topics the real Pixhawk 6C produces — is
> unchanged and is why the stack is what it is. Read it for the *why*.
>
> **Its UE5.5 specifics are superseded by UE5.8**, along with its ROS 2 Humble
> recommendation and its weeks-based phase calendar. Those corrections are recorded inside
> the document itself (see "Decisions taken on this document") rather than edited over the
> original, so the research record and the decision that overrode it are both visible. It
> also compares against the Gazebo and Isaac Sim stacks as live alternatives; **those are
> retired and this repo no longer contains them.**
>
...
> longer resolve. Every measurement and citation is preserved unchanged.
>
> **The repo today is one simulator: Unreal Engine 5.8 + Cosys-AirSim + PX4 SITL + ROS 2
> Jazzy.** Context: [`../README.md`](../README.md). Task-ID renames:
> [`../id-map.md`](../id-map.md). The live backlog: [`../../todo.md`](../../todo.md). The
